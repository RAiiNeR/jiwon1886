...
interface ImgCarouselProps {
    data: Array<string>; // 각 이미지를 문자열로 받아옵니다.
}
                    ...
                    <div className="owl-stage img-carousel" style={{ transform: "translate3d(0px, 0px, 0px)", transition: "all", width: "4830px" }}>
                        {
                            data.map((e, i) => (
                                <div key={i} className={`owl-item img-item ${i == 0 ? 'active' : ''}`} style={{ width: `${carouselItemWidth}px` }}>
                                    <div className="item">
                                        {/* 서버에서 이미지 URL을 가져오는 형식으로 수정 */}
                                        <div className="hotel-img" style={{ backgroundImage: `url(${process.env.REACT_APP_FILES_URL}/img/hotels/${e})` }}></div>
                                    </div>
                                </div>
                            ))
                        }
