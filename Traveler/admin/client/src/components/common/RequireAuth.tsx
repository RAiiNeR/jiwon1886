...
const RequireAuth: React.FC<RequireAuthProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('adminToken'); // 브라우저 localStorage에서 token키로 저장된 값 가져옴
    ...
    if (!token) { // 인증토큰 확인 -> 토큰이 없으면
      ...
      navigate(`/travelerAdmin/login/login?redirect=${encodeURIComponent(location.pathname)}`);
    }
