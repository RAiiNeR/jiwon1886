...
const VoteModal: React.FC<VoteModalProps> = ({ isOpen, onClose, onSave, cbnum }) => {
    ...
    const [options, setOptions] = useState<VoteOption[]>([
        { text: '', image: null, image_url: null }, // 기본옵션1
        { text: '', image: null, image_url: null }, // 기본옵션2
    ]);
    ...
    const [showResults, setShowResults] = useState<'after_vote' | 'always'>('after_vote'); // 투표결과 표시 조건
    ...
    const handleSave = () => {
        const pollData = {
            ...
            options: options.map((option) => ({
                text: option.text,
                image_url: option.image_url || null, // null도 허용(이미지 URL 포함)
            })),
            allow_multiple,
            anonymous,
            autoEndType,
    ...
    const handleCancel = () => {
        ...
        setAllow_multiple(false);
        setAnonymous(false);
        setAutoEndType('none');
        setEnd_date('');
        setMax_participants(0);
    ...
    return (
        <div className="complevote vote-modal-overlay">
            <div className="vote-modal-content">
                            ...
                            <div className="image-container">
                                {option.image_url ? (
                                    <div className="image-preview">
                                        ...
                                        />
                                        <button className="image-remove-button" onClick={() => handleImageRemove(index)}>×</button>
                                    </div>
                                ) : (
                ...
                <button type="button" onClick={handleAddOption} className="vote-add-option" disabled={options.length > 4 ? true : false}>
                    항목 추가
                </button>
                ...
                <div className='vote-btn-box'>
                    <button className="vote-cancel-button" onClick={handleCancel}>취소</button>
                    <button className="vote-save-button" onClick={handleSave}>저장</button>
                </div>
            </div>
        </div>
    );
