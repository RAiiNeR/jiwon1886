...
public class SurveyService {
    ...
    public Page<Map<String,String>> getList(int page, int size){
        // startRow와 endRow 계산
        int startRow = (page - 1) * size + 1;
        int endRow = startRow + size - 1;
        List<Map<String,String>> maps = surveyRepository.findAllByOrderByNumDesc(startRow, endRow); // 모든 설문조사 조회
        int totalElements = surveyRepository.countAll();
        return new PageImpl<>(maps, PageRequest.of(page - 1, size), totalElements);
    }
}
