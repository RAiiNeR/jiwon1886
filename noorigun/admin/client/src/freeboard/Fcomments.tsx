...
const Fcomments: React.FC<FcommentsProps> = ({ fbnum }) => {
  ...
  const fetchComments = async (page: number) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACK_END_URL}/api/fcomm/${fbnum}/paged`,
        {
          params: {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BACK_END_URL}/api/fcomm`, {
        fbnum,
      ...
      try {
        await Promise.all(
          selectedFcomments.map((num) =>
            axios.delete(`${process.env.REACT_APP_BACK_END_URL}/api/fcomm/${num}`)
          )
        );
