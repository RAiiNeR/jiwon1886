...
const ProgramList: React.FC = () => {
  ...
  const getProgramList = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACK_END_URL}/api/program/programList`);
  ...
  useEffect(() => {
    ...
    setFilteredPosts(programList.slice(startIndex, endIndex)); // 현재 페이지 데이터만 필터링

    // 페이지 번호 처리
    const newStartPage = Math.floor((currentPage - 1) / 5) * 5 + 1;
    setStartPage(newStartPage);
    setEndPage(Math.min(newStartPage + 4, totalPages));
  }, [programList, currentPage, totalPages]);
  ...
  const handleDelete = async () => {
    try {
      ...
      for (const itemNum of selectedItems) {
        await axios.delete(`${process.env.REACT_APP_BACK_END_URL}/api/program?num=${itemNum}`);
      }
      // 삭제 후 프로그램 목록 갱신
      getProgramList();
      ...
      <div className="ProgramList">
        <div>
          ...
          <div className="action-buttons">
            <Link className="btn btn-success" to="/noorigun/program/new">
              등록
            </Link>
