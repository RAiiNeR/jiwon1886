...
const MapForm: React.FC<Mapdata> = ({ onChange, onClose }) => {
  const [place, setPlace] = useState('');  // 주소 입력
  ...
  return (
    <div>
      <div className='d-flex'>
        ...
        <button type="button" className='btn btn-info' onClick={() => geocodeAddress(place)}>
          주소로 위치 찾기
        </button>
      </div>
            ...
            <tbody>
              {
                placeList.map((place, index) => (
                  <tr key={index}>
                    ...
                    <td><button type='button' onClick={_ => handleSelectPlace(place)} className='map-btn' style={{margin:'0px'}}>저장</button></td>
                  </tr>
                ))
              }
            </tbody>
