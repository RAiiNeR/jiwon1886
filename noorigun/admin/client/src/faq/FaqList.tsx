...
const FaqList: React.FC = () => {
  ...
  const fetchFaqList = async (page: number, search: string) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACK_END_URL}/api/faq`, {
        params: {
          page: page,
          size: size,
          searchValue: search
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      for (const itemNum of selectedFaqs) {
        await axios.delete(`${process.env.REACT_APP_BACK_END_URL}/api/faq?num=${itemNum}`);
      }
      // 선택된 항목들을 목록에서 제거
                ...
                <tr>
                  ...
                  <td>
                    <button className="btn btn-primary">
                      <Link to={'/noorigun/faq/new'}>글쓰기</Link>
                    </button>
                  </td>
                </tr>
              </tfoot>
