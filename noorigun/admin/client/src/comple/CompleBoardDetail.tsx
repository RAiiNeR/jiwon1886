...
const CompleBoardDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACK_END_URL}/api/comple/detail?num=${num}`
        );
    ...
    const fetchReplies = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BACK_END_URL}/api/comple/reply/${num}`
        );
  ...
  const handleDelete = async () => {
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_BACK_END_URL}/api/comple/delete?num=${num}`
        );
        alert("게시글이 삭제되었습니다.");
        navigate("/noorigun/comple");
      } catch (error) {
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BACK_END_URL}/api/comple/reply/add`, {
        cbnum: num,
        content: newReply,
      ...
      <div style={{ padding: '50px' }}>
        <div className="admin-comple-detail">
            ...
            <div className="image-preview-container">
              ...
              <div className="image-gallery">
                {boardDetail.imgNames.map((img, index) => (
                  <img
                    key={index}
                    src={`${process.env.REACT_APP_BACK_IMG_URL}/${img}`} // 서버 이미지 경로
                    alt={`첨부 이미지 ${index + 1}`}
          ...
          <div className="action-buttons">
            ...
            <button onClick={() => navigate("/noorigun/comple")} className="list-btn">
              목록으로
            </button>
