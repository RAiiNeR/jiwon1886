...
const PromoteBoardList: React.FC = () => {
  ...
  const getPromoteBoardList = async (page: number, title: string) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACK_END_URL}/api/promote/promoteList`, {
        params: {
          page: page,
          size: itemsPerPage,
          searchValue:title
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (const itemNum of selectedItems) {
        await axios.delete(`${process.env.REACT_APP_BACK_END_URL}/api/promote?num=${itemNum}`);
      }
      // 선택된 항목들을 목록에서 제거
              ...
              {promoteBoardList.length === 0 ? (
                ...
                promoteBoardList.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <Link to={`/noorigun/promote/${item.num}`}>{item.title}</Link>
                    </td>
                    <td>{item.writer}</td>
                    <td>{item.content}</td>
                    <td>{item.hit}</td>
              ...
              <tr>
                ...
                <td colSpan={2}>
                  ...
                  <div className="action-buttons">
                    <Link className="btn btn-success" to="/noorigun/promote/new">
                      등록
                    </Link>
