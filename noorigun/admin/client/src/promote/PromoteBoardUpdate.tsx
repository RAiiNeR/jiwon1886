...
const PromoteBoardUpdate: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_BACK_END_URL}/api/promote/detail?num=${num}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(`${process.env.REACT_APP_BACK_END_URL}/api/promote`, data, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            alert('수정이 완료되었습니다!');
            navigate(`/noorigun/promote/${num}`); // 수정 후 상세 페이지로 이동
        } catch (error: any) {
