...
const QnaBoardDetail: React.FC = () => {
  ...
  useEffect(() => {
    const getDetail = async () => {
      try {
        const response = await axios.get('http://localhost:82/noorigun/api/qnaboard/detail?num=' + num);
        setQnadetail(response.data);
        if (response.data.parentNum) setIsVisiable(false);// 부모글인 경우 답글 버튼 숨김

        const replyResponse = await axios.get('http://localhost:82/noorigun/api/qnaboard/replies?parentNum=' + num);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await axios.delete('http://localhost:82/noorigun/api/qnaboard?num=' + num);
  ...
  const checkReply = async () => {
    try {
      ...
      console.log(response.data)
      if (response.data === 0) {
        navigate(`/qna/new?parentNum=${num}`);
      } else {
        alert('이미 답글이 있습니다.');
      }
    } catch (error) {
      ...
      <div className="QnAD-container">
        <div className="QnAD-card">
          ...
          <div className="QnAD-card-footer">
            <Link to={"/qna"} className="QnAD-btn QnAD-btn-primary me-2">
              목록
            </Link>
