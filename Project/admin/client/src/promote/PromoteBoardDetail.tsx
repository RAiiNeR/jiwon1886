...
const PromoteBoardDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:82/noorigun/api/promote/detail?num=${currentNum}`);
                console.log('Fetched Data:', response.data); // 데이터 확인을 위해 로그 추가
                setPromoteBoard(response.data);

                // 이전 글 정보 가져오기
                if (currentNum > 1) {
                    const prevResponse = await axios.get(`http://localhost:82/noorigun/api/promote/detail?num=${currentNum - 1}`);
                ...
                // 다음 글 정보 가져오기
                const nextResponse = await axios.get(`http://localhost:82/noorigun/api/promote/detail?num=${currentNum + 1}`);
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(`http://localhost:82/noorigun/api/promote?num=${currentNum}`);
            navigate('/promote'); // 삭제 후 메인 페이지로 이동
        } catch (error) {
    ...
    // 글 수정 페이지로 이동
    const handleUpdate = () => {
        navigate(`/promote/update/${currentNum}`); // 수정 페이지로 이동
    };
