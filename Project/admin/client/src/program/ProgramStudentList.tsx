...
const ProgramStudentList: React.FC = () => {
  ...
  const getProgramList = async () => {
    try {
      const response = await axios.get(`http://localhost:82/noorigun/api/program/memberlist?num=${num}`);
  ...
  return (
    <div className='StudentList'>
      ...
      <Link className="btn btn-success" to="/program">메인페이지</Link>
      <Link className='btn btn-primary' to={`/program/mail/${num}`} state={ {emails: getSelectedEmails() }}>메일발송</Link>
    </div>
  );
};
