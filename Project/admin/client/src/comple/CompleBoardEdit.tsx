...
const CompleBoardEdit: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:82/noorigun/api/comple/detail?num=${num}`);
        //폼 데이터 상태 업데이트
        setFormData({
          ...response.data,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    //서버에 내용수정 요청
    try {
      axios.put(`http://localhost:82/noorigun/api/comple/update`, null, {
        params: {
          num: formData.num,
      ...
      alert("게시글이 수정되었습니다.");
      navigate(`/comple/${num}`);//수정되면 목록페이지로 이동
    } catch (err) {
        ...
        <div className="admin-comple-edit">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <Link to={`/comple/${num}`} className="btn btn-danger">취소</Link>
            </div>
          </form>
        </div>
      </div>
