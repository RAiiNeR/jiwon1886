...
//댓글 목록 가져오기
  const fetchComments = async (page: number) => {
    try {
      const response = await axios.get(
        `http://localhost:82/noorigun/api/ccomm/${cbnum}/paged`,
        {
          params: {
 ...
 //새 댓글 작성처리
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post("http://localhost:82/noorigun/api/ccomm", {
        cbnum,
        comments: newComment,
        writer: "관리자",
      });
      ...
      try {
        await Promise.all(
          selectedComments.map((num) =>
            axios.delete(`http://localhost:82/noorigun/api/ccomm/${num}`)
          )
        );
