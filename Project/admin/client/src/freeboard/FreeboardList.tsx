...
const FreeBoardList: React.FC = () => {
  ...
  const filePath = 'http://localhost:82/noorigun/uploads/';//이미지 파일경로

  const getFreeList = async (page: number, title: string) => {
    try {
      const response = await axios.get('http://localhost:82/noorigun/api/freeboard', {
        params: {
          page,
          size,
          searchValue: title,
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      for (const itemNum of selecttopics) {
        await axios.delete(`http://localhost:82/noorigun/api/fcomm?num=${itemNum}`);
        await axios.delete(`http://localhost:82/noorigun/api/freeboard?num=${itemNum}`);
      }
      // 선택된 항목들을 목록에서 제거
      getFreeList(page, title);
      setSelectTopics(new Set());
