...
const SuggestionDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const suggestionResponse = await axios.get(`http://localhost:82/noorigun/api/suggestion/${num}`);
                ...
                const pollResponse = await axios.get(`http://localhost:82/noorigun/api/admin/polls/${num}`);
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(
                `http://localhost:82/noorigun/api/suggestion?num=${num}`
                 );
                alert("게시글이 삭제되었습니다.")
            navigate('/suggest');
        } catch (error) {
    ...
    const handleStateChange = async (newState: string) => {
        try {
            await axios.put('http://localhost:82/noorigun/api/suggestion/state', {
                num,
                state: newState,
            });
            setState(newState);
