...
const Scomments: React.FC<ScommentsProps> = ({ sbnum }) => {
  ...
  const fetchComments = async (page: number) => {
    try {
      const response = await axios.get(
        `http://localhost:82/noorigun/api/scomm/${sbnum}/paged`,
        {
          params: {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post("http://localhost:82/noorigun/api/scomm", {
        sbnum,
      ...
      try {
        await Promise.all(
          selectedScomments.map((num) =>
            axios.delete(`http://localhost:82/noorigun/api/scomm/${num}`)
          )
        );
